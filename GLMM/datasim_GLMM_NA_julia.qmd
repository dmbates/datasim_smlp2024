...
#| output: false
using MixedModels
using RCall       # call R from within a Julia session

R"""              # evaluate expressions in R
library(lme4)
library(lattice)
set.seed(1614)
"""
...
:::{.callout-note}
I haven't worked out how to get the plots from an {r} block to display.
:::
...
# Analysis with `MixedModels`

```{julia}
#| label: analysis

m1 = let f = @formula(resp ~ representation * veracity + (1|id) + (1|item))
    d = rcopy(R"dat")
    contrasts = Dict(
    ...
    fit(MixedModel, f, d, Bernoulli(); contrasts, progress)
end
```
