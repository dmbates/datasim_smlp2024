...
```{julia}
m1 = let f = @formula(resp ~ framing * trend * spatialdist + (trend + framing + spatialdist | subj_id))
  d = rcopy(R"dat")
...
```{julia}
println(m1)  # for those who like to see the whole story
```

The random-effects covariance matrix is of rank 3, not 4.
```{julia}
m1.rePCA
```
